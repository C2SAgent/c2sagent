...
class EnhancedServerToolManager:
        ...
        if existing_server:
            ...
            if not tool_exists:
                ...
                self.server_table.update(
                    {"tools": updated_tools}, Server.name == server_name
                )
        else:
            # 服务器不存在，创建新记录
    ...
    def get_servers_by_tool_name(self, tool_name: str) -> List[str]:
        """根据工具名称获取服务器列表"""
        Server = Query()
        servers = self.server_table.search(
            Server.tools.any(lambda tool: tool["name"] == tool_name)
        )
        return [server["name"] for server in servers]

    def update_tool_handler(
        self, server_name: str, tool_name: str, new_handler: Dict[str, Any]
    ) -> bool:
        """更新工具的handler信息"""
        Server = Query()
        ...
        if existing:
            ...
            for tool in tools:
                if tool["name"] == tool_name:
                    ...
                    self.server_table.update(
                        {"tools": tools}, Server.name == server_name
                    )
                    return True
        return False
    ...
    def search_tools_by_keyword(self, keyword: str) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            if (
                keyword_lower in tool["name"].lower()
                or keyword_lower in tool.get("description", "").lower()
            )
        ]
