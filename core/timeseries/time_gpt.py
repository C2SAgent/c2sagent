...
class TimeGPT:
    ...
    def predict(
        ...
        time_fcst_df = self.nixtla_client.forecast(
            df=df, h=h, time_col=time_col, target_col=target_col
        )
        fig = self.nixtla_client.plot(
            df[-h * 2 :], time_fcst_df, time_col=time_col, target_col=target_col
        )
        buf = io.BytesIO()
...
if __name__ == "__main__":
    ...
    time_fcst_df, fig_data = predictor.predict(
        df=df, h=12, freq="MS", time_col="date", target_col="OT"
    )
