...
async def login_for_access_token(
    ...
    try:
        result = await db.execute(
            select(models.UserConfig)
            .where(models.UserConfig.name == form_data.username)
        )
...
async def refresh_access_token(
    ...
    try:
        ...
        result = await db.execute(
            select(models.UserConfig)
            .where(models.UserConfig.name == username)
        )
...
async def register_user(
    user_data: model_auth.UserCreate,
    db: AsyncSession = Depends(get_db)
):
    """异步用户注册"""
    try:
        ...
        result = await db.execute(
            select(models.UserConfig)
            .where(models.UserConfig.name == user_data.name)
        )
