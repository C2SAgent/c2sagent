...
async def do_agent_corr_mcp(
    ...
    try:
        ...
        agent = await db.fetch_one(
            models.AgentCard, id=agent_card_id, user_id=current_user.id
        )
        if not agent:
            raise HTTPException(
                status_code=Status.HTTP_404_NOT_FOUND,
        ...
        await db.insert(
            ...
            {"agent_card_id": agent_card_id, "mcp_server_id": mcp_server_id},
        )
        logger.info(
            f"User {current_user.id} linked agent {agent_card_id} to MCP {mcp_server_id}"
        )
        return BaseResponse()
    except Exception as e:
...
async def do_agent_discorr_mcp(
    ...
    try:
        ...
        logger.info(
            f"User {current_user.id} unlinked agent {agent_card_id} from MCP {mcp_server_id}"
        )
        return BaseResponse()
    except Exception as e:
...
async def do_find_mcp(
    ...
    try:
        ...
        relation = await db.fetch_one(
            models.AgentCardAndMcpServer, agent_card_id=agent_card_id
        )
