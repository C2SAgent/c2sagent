...
#     def __init__(self, agent_executor, task_store: InMemoryTaskStore):
#         self.agent_executor_factory = agent_executor
...
# # 在 main.py 末尾添加：
# def get_app():
#     base_agent_card = AgentCard(...)  # 你的初始化代码
#     task_store = InMemoryTaskStore()
#     def handler_factory(agent_index: Optional[str] = None):
#         executor = LifeAgentExecutor(agent_index)
#         return A2ARequestHandler(lambda: executor, task_store)
#     return DynamicAgentApp(base_agent_card, handler_factory).build()
